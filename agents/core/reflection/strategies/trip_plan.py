"""Reflexion Agent strategies for Trip Planning."""
...
from agents.eval.trip_planning import NPTripPlanningSolution, tp_is_correct, parse_response
...
class ReflexionCoTTripPlanStrategy(ReflexionCoTBaseStrategy):
    """A strategy class for Trip Planning benchmarks using the ReflexionCoT agent.
    ...
    def generate_observation(
        self, action_type: str, query: str, key: NPTripPlanningSolution
    ) -> Tuple[bool, str]:
        ...
        if action_type.lower() == "finish":
            self._finished = True
            self._answer = query
            if tp_is_correct(self._answer, key):
        ...
        return tp_is_correct(self._answer, key), obs
    ...
    def halting_condition(self, idx: int, key: NPTripPlanningSolution, **kwargs: Any) -> bool:
        ...
        return tp_is_correct(self._answer, key) or idx >= max_trials
    ...
    def reflect_condition(
        self, idx: int, reflect_strategy: Optional[str], key: NPTripPlanningSolution
    ) -> bool:
        ...
        return (
            idx > 0
            and not tp_is_correct(self._answer, key)
            and reflect_strategy is not None
        )

class SelfReflectionCoTTripPlanStrategy(ReflexionCoTTripPlanStrategy):
    ...
    def generate_observation(
        ...
        if action_type.lower() == "finish":
            self._finished = True
            self._answer = query
            obs = f"Generated answer is {str(parse_response(query))}."
...
class ReflexionCoTNPTripPlanningStrategy(ReflexionCoTTripPlanStrategy):
    """A strategy class for the Natural Plan Meeting Planning benchmark using the ReflexionCoT agent."""

    pass

class SelfReflectionCoTNPTripPlanningStrategy(SelfReflectionCoTTripPlanStrategy):
