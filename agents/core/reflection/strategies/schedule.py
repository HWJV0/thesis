"""Reflexion Agent strategies for Scheduling."""
...
from agents.eval.calendar_scheduling import cs_is_correct, find_response
...
class ReflexionCoTSchedulingStrategy(ReflexionCoTBaseStrategy):
    """A strategy class for Scheduling benchmarks using the ReflexionCoT agent.
    ...
    def generate_observation(
        ...
        return cs_is_correct(query, key), obs
    ...
    def halting_condition(self, idx: int, key: str, **kwargs: Any) -> bool:
        ...
        return cs_is_correct(self._answer, key) or idx >= max_trials
    ...
    def reflect_condition(
        ...
        return (
            idx > 0
            and not cs_is_correct(self._answer, key)
            and reflect_strategy is not None
        )

class SelfReflectionCoTSchedulingStrategy(ReflexionCoTSchedulingStrategy):
    """A strategy class for Scheduling benchmarks using the SelfReflectionCoT agent.
    ...
    def generate_observation(
        ...
        if action_type.lower() == "finish":
            self._finished = True
            self._answer = query
            obs = f"Generated answer is {find_response(self._answer)}"
...
class ReflexionCoTNPCalendarSchedulingStrategy(ReflexionCoTSchedulingStrategy):
    """A strategy class for the Natural Plan Calendar Scheduling benchmark using the ReflexionCoT agent."""

    pass

class SelfReflectionCoTNPCalendarSchedulingStrategy(SelfReflectionCoTSchedulingStrategy):
    """A strategy class for the Natural Plan Calendar Scheduling benchmark using the SelfReflectionCoT agent."""
