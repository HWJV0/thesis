...
from agents.utils.docstore import DocstoreExplorer
...
class ReflexionCoTQAStrategy(ReflexionCoTBaseStrategy):
    ...
    def generate_observation(
        ...
        if action_type.lower() == "finish":
            self._finished = True
            self._answer = query
            if qa_is_correct(self._answer, key):
                obs = "The generated answer is CORRECT."
            else:
                obs = "The generated answer is INCORRECT."
        else:
            obs = "Invalid action type, please try again."
        self._scratchpad += obs

        return qa_is_correct(self._answer, key), obs
    ...
    def halting_condition(
        ...
        **kwargs: Any,
        ...
        return qa_is_correct(self._answer, key) or idx >= max_trials
    ...
    def reflect_condition(
        ...
        """
        return idx > 0 and not qa_is_correct(self._answer, key) and reflect_strategy is not None


class SelfReflectionCoTQAStrategy(ReflexionCoTQAStrategy):
    """A strategy class for QA benchmarks using the ReflexionCoT agent.
    ...
    def generate_observation(
        ...
        if action_type.lower() == "finish":
            self._finished = True
            self._answer = query
            obs = f"The generated answer is {self._answer}."
        else:
            obs = "Invalid action type, please try again."
        self._scratchpad += obs
...
class ReflexionCoTGSM8KStrategy(ReflexionCoTQAStrategy):
    """A strategy class for the GSM8K benchmark using the ReflexionCoT agent."""

    pass

class SelfReflectionCoTGSM8KStrategy(SelfReflectionCoTQAStrategy):
    """A strategy class for the GSM8K benchmark using the ReflexionCoT agent."""
