"""Reflexion Agent strategies for Meeting Planning."""
...
from agents.eval.meeting_planning import NPMeetingPlanningSolution, mp_is_correct, parse_text_plan
...
class ReflexionCoTMeetingPlanStrategy(ReflexionCoTBaseStrategy):
    """A strategy class for Meeting Planning benchmarks using the ReflexionCoT agent.
    ...
    def generate_observation(
        self, action_type: str, query: str, key: NPMeetingPlanningSolution
    ) -> Tuple[bool, str]:
        ...
        return mp_is_correct(query, key), obs
    ...
    def halting_condition(self, idx: int, key: NPMeetingPlanningSolution, **kwargs: Any) -> bool:
        ...
        return mp_is_correct(self._answer, key) or idx >= max_trials
    ...
    def reflect_condition(
        self, idx: int, reflect_strategy: Optional[str], key: NPMeetingPlanningSolution
    ) -> bool:
        ...
        return (
            idx > 0
            and not mp_is_correct(self._answer, key)
            and reflect_strategy is not None
        )

class SelfReflectionCoTMeetingPlanStrategy(ReflexionCoTMeetingPlanStrategy):
    ...
    def generate_observation(
        ...
        if action_type.lower() == "finish":
            self._finished = True
            self._answer = query
            obs = f"Generated answer is {self._answer}"
...
class ReflexionCoTNPMeetingPlanningStrategy(ReflexionCoTMeetingPlanStrategy):
    """A strategy class for the Natural Plan Meeting Planning benchmark using the ReflexionCoT agent."""

    pass

class SelfReflectionCoTNPMeetingPlanningStrategy(SelfReflectionCoTMeetingPlanStrategy):
