...
class NPMeetingPlanningSolution:
    """
    Class to encapsulate all the parameters required for a Meeting Planning solution.

    Attributes:
        start_location (str): The starting location of the plan.
        initial_time (str): The initial time of the plan.
        constraints (list): The constraints for the plan.
        dist_matrix (list): The distance matrix for the plan.
        golden_plan (str): The correct, golden plan.
    """
    def __init__(self, start_location: str, initial_time: str, constraints: list, dist_matrix: list, golden_plan: str):
...
def validator_from_text(
  ...
  # travel_pattern = re.compile(r'you(?:.*)travel(?:.*)to\s+([a-zA-Z\s]+)\s+in\s+(\d+)\s+minutes(?:.*arrive(?:.*at\s+(\d+:\d+[APMapm]+))?)?', re.IGNORECASE)
  ...
  for step in plan:
    try:
      if step.startswith("You start"):
        continue
      elif step.startswith("You travel") or step.startswith("You then travel"):
      ...
      elif step.startswith("You wait") or step.startswith("You then wait"):

        raw_end_time = step.split("until ")[1].split(".")[0].strip()
        ...
        cur_time = end_time
      elif step.startswith("You meet") or step.startswith("You then meet"):
...
def mp_is_correct(query, key: NPMeetingPlanningSolution) -> bool:
    """
    Checks if the predicted plan matches the golden plan based on individual parameters.

    Args:
      key (NPMeetingPlanningSolution): The NPMeetingPlanningSolution object containing the parameters required for the Meeting Planning task.

    Returns:
        bool: True if the predicted plan matches the golden plan, False otherwise.
    """

    # Process constraints
    processed_constraints = process_constraints(key.constraints)
    
    # Parse the predicted and golden plans
    parsed_pred_plan = parse_text_plan(query)
    parsed_golden_plan = parse_text_plan(key.golden_plan)

    # Validate the plans using the validator function
    pred_score = validator_from_text(parsed_pred_plan, processed_constraints, key.start_location, key.initial_time, key.dist_matrix)
    golden_score = validator_from_text(parsed_golden_plan, processed_constraints, key.start_location, key.initial_time, key.dist_matrix)

    # Return whether the predicted score matches the golden score
