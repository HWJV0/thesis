...
#
"""Eval Functions for Calendar Scheduling."""
...
def find_response(response: str):
  """Parse the response.
...
def compute_solve_rate(responses: list[str], solutions: list[str]):
  ...
  for r, s in zip(responses, solutions):
    r_day, r_start_hour, r_end_hour = parse_response(r)
    s_day, s_start_hour, s_end_hour = parse_response(s)
    if (
        r_day == s_day
...
def cs_is_correct(response: str, solution: str) -> bool:
    """Checks if the response is correct based on the solution.

    Args:
        response (str): The response to check.
        solution (str): The solution to compare against.

    Returns:
        bool: True if the response is correct, False otherwise.
    """
    r_day, r_start_hour, r_end_hour = parse_response(response)
    s_day, s_start_hour, s_end_hour = parse_response(solution)
    if (
        r_day == s_day
        and r_start_hour == s_start_hour
        and r_end_hour == s_end_hour
